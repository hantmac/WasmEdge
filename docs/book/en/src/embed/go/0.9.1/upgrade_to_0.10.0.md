...
1. Merged the `ImportObject` into the `Module`.
   ...
   * `(*wasmedge.ImportObject).CreateWASI()` is changed to `(*wasmedge.Module).CreateWASI()`.
   * `(*wasmedge.ImportObject).CreateWasmEdgeProcess()` is changed to `(*wasmedge.Module).CreateWasmEdgeProcess()`.
